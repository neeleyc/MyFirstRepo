
#Source http://ordinaryhumanlanguage.ca/blog/wp-content/uploads/2017/07/Mad-Libs-Example.jpg

print ("Give me a noun")
noun1 = input()

print ("Give me a verb")
verb1 = input ()

print ("Give me a person in the room")
person1 = input ()

print ("Give me a part of the body")
body1 = input ()

print ("Give me an adjective")
adj1 = input ()

print ("Give me a plural noun")
pnoun1 = input ()

print ("Give me a type of liquid")
liquid1 = input ()

print ("Give me a plural noun")
pnoun2 = input ()

print ("Give me an adjective")
adj2 = input ()

print ("Give me a part of the body")
body2 = input ()

print ("Give me a person in the room")
person2 = input ()

print ("Give me a verb")
verb2 = input ()

print ("Give me a noun")
noun2 = input()

print ("Give me a noun")
noun3 = input()

print ("Give me a noun")
noun4 = input()

print ("Give me a noun")
noun5 = input()

print ("Give me a noun")
noun6 = input()

### Begin Mad Lib ###
print ("It was Thanksgiving, and the scent of succulent roast " + noun1 + "wafterd through my house.")
print (person1 + ", its time to " + verb1+ "! my mother called")
print ("I couldn't wait to get my " + body1 + "on that " + adj1 + "Thanksgiving meal.")
print ("My family sat around the dining-room " + noun2 + ".")
print ("The table was laid out with every kind of " + noun3 + " imaginable")
print ("There was a basket of hot buttered " + pnoun1 + "and glasses of sparkling" + liquid1)
print ("The" + adj2 + "turkey sat, steaming, next to a tureen of" + noun4 + "gravy")
print ("A bowl of rudy-red " + noun4 + "sauce, a sweet- " + noun5 + " casserole, and a dish of mashed" + pnoun2 + "tempted my taste buds.")
print ("But the dish I looked forward to most was Grandma " + person1 + " 's famous" + noun6 + "pie.")
print ("Thanksgiving is my favorite holiday, " + body2 + " down")
       


